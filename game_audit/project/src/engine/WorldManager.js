...
class WorldManager {
  ...
  enterDungeon(dungeonId) {
    ...
    window.GameEvents.emit('world:enterDungeon', { dungeonId, biome: dungeon.biome });
    return {
      x: dungeon.entranceX,
      y: dungeon.entranceY,
    };
  ...
  exitDungeon() {
    ...
    if (dInfo) {
      window.GameEvents.emit('world:exitDungeon', {});
  ...
  _updateWeather(dt, player) {
    ...
    if (this._weatherTimer >= this._nextWeatherChange) {
      ...
      window.GameEvents.emit('world:weatherChange', { weatherType: this.targetWeather, intensity: this.targetWeatherIntensity });
    }
  ...
  _updateDayNight(dt) {
    ...
    if (prevTimeOfDay !== this.timeOfDay) {
      window.GameEvents.emit('world:timeChange', { timeOfDay: this.timeOfDay, worldTime: this.worldTime });
    }
  }
  ...
  update(dt, player) {
    ...
    if (!this.activeDungeon && player) {
      ...
      if (newBiome !== this.currentBiome) {
        ...
        window.GameEvents.emit('world:biomeChange', { fromBiome: oldBiome, toBiome: newBiome });
      }
    }
