...
class Entity {
  ...
  /**
   * Squared distance to another entity (avoids sqrt, cheaper for comparisons).
   * @param {Entity|{x: number, y: number}} other - Target position.
   * @returns {number} Squared distance in pixels^2.
   */
  distanceToSq(other) {
    const dx = this.x - other.x;
    const dy = this.y - other.y;
...
class Player extends Entity {
  ...
  _applyPhysics(dt, world) {
    ...
    if (this.scale <= 0.3 && this.onGround) {
      ...
      const gapCheckY = this.y - hb.height - world.tileSize;
  ...
  attack() {
    ...
    // Emit combo event
    window.GameEvents.emit('combat:combo', { count: this.comboCount + 1, multiplier: [1.0, 1.3, 1.6][this.comboCount] });

    // Emit hitstop for attack feel
    window.GameEvents.emit('system:hitstop', { duration: 0.02 });
  }
  ...
  releaseCharge() {
    ...
    window.GameEvents.emit('system:hitstop', { duration: 0.05 + chargeRatio * 0.1 });

    window.GameEvents.emit('system:screenshake', { intensity: 3 + chargeRatio * 5, duration: 0.2 + chargeRatio * 0.2 });
  }
  ...
  dodge(direction) {
    ...
    window.GameEvents.emit('combat:dodge', { entity: this, direction });
  }
  ...
  onParrySuccess(attacker) {
    ...
    window.GameEvents.emit('combat:parry', { attacker, defender: this, position: { x: this.x, y: this.y } });

    window.GameEvents.emit('system:hitstop', { duration: 0.15 });

    window.GameEvents.emit('system:screenshake', { intensity: 4, duration: 0.25 });
  }
  ...
  shrink(world) {
    ...
    if (!inZone && !this.canShrinkAnywhere) {
      window.GameEvents.emit('system:notification', { message: 'Find a Shrink Zone or the Whispering Seed to shrink here.', type: 'warning' });
      return;
    }
    ...
    window.GameEvents.emit('player:scaleChange', { fromScale: this.scale, toScale: 0.25, duration: 0.8 });
  }
  ...
  grow() {
    ...
    window.GameEvents.emit('player:scaleChange', { fromScale: this.scale, toScale: 1.0, duration: 0.8 });
  }
  ...
  takeDamage(amount, source) {
    ...
    window.GameEvents.emit('system:screenshake', { intensity: 5, duration: 0.2 });
  ...
  heal(amount) {
    ...
    if (healed > 0) {
      window.GameEvents.emit('system:notification', { message: `+${healed} HP`, type: 'success' });
    }
  }
  ...
  addItem(item) {
    ...
    window.GameEvents.emit('player:itemPickup', { itemId: item.id, itemName: item.name || item.id });
  }
  ...
  startQuest(quest) {
    ...
    window.GameEvents.emit('quest:start', { questId: quest.id });
  }
  ...
  updateQuestProgress(questId, progress) {
    ...
    window.GameEvents.emit('quest:update', { questId, objective: quest.objective, progress });
  ...
  completeQuest(questId) {
    ...
    window.GameEvents.emit('quest:complete', { questId, rewards: quest.reward ? [quest.reward] : [] });
  }
