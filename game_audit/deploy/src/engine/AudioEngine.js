...
class AudioEngine {
  ...
  _scheduleLayerNotes() {
    ...
    // ---- Rhythm layer ----
    if (this.layers.rhythm.volume > 0.01 && this.layers.rhythm.active) {
      this._scheduleRhythm(now, beatLen, theme);
    }

    // ---- Bass layer ----
    if (this.layers.bass.volume > 0.01 && this.layers.bass.active) {
      this._scheduleBass(now, beatLen, theme);
    }

    // ---- Melody layer ----
    if (this.layers.melody.volume > 0.01 && this.layers.melody.active) {
